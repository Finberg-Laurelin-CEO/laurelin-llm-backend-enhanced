...
export class ProcessingService {
  ...
  private async determineModelProvider(request: LLMRequest): Promise<'openai' | 'google' | 'aws-bedrock'> {
    ...
    if (this.config.getConfig().abTestEnabled) {
      ...
      if (variant) {
        return variant as 'openai' | 'google' | 'aws-bedrock';
      }
      ...
      return assignedVariant as 'openai' | 'google' | 'aws-bedrock';
    }

    // Default to configured provider
    return this.config.getConfig().abTestDefaultProvider as 'openai' | 'google' | 'aws-bedrock';
  }
